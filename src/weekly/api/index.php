...
function updateWeek(PDO $db, array $data): void
{
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // rowCount() can be 0 if the submitted values are identical to what
        // is already stored — treat that as a success rather than an error.
...
function createComment(PDO $db, array $data): void
{
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // Fetch the newly created comment to return it in full.
        $fetch = $db->prepare(
